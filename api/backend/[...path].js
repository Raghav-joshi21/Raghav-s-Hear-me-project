...
export default async function handler(req, res) {
  ...
  // In Vercel, for [...path].js, the path segments are in req.query.path (array)
  // Example: /api/backend/room/ABC123 -> req.query.path = ['room', 'ABC123']
  // Example: /api/backend/api/azure/token -> req.query.path = ['api', 'azure', 'token']
  let backendPath = "";
  ...
  if (req.query.path) {
    // req.query.path is an array: ['room'] or ['room', 'ABC123'] or ['api', 'azure', 'token']
    backendPath = Array.isArray(req.query.path)
      ? req.query.path.join("/")
      : req.query.path;
    console.log('[Proxy Debug] Extracted path from req.query.path:', backendPath);
  } else {
    // Fallback: extract from URL
    const pathMatch = req.url.match(/^\/api\/backend\/(.*?)(\?|$)/);
    if (pathMatch && pathMatch[1]) {
      backendPath = pathMatch[1];
      console.log('[Proxy Debug] Extracted path from URL regex:', backendPath);
    } else {
      console.warn('[Proxy Debug] Could not extract path from URL:', req.url);
    }
  }
